...
async function handleApproval(args, ticket, approvalRule, isUpdated = false, requestedItems) {
  ...
  const placeholder = { ticket, requested_item: requestedItems };
  let requesters = [];
  let approver = "";
  ...
  if (requesters && requesters.length > 0) {
    if (Boolean(approvalRule.is_not_send_approval)) {
      ...
      sendEmailTemplate(
        approvalRule.email_template_code,
        placeholder,
        requesters
          .map((r) => {
    ...
    await Promise.all(
      ...
      .then(async function () {
        ...
        if (isUpdated) sendEmailTemplate(approvalRule.approval_email_template_code, placeholder, args.data.requester.email);
        sendEmailTemplate(
          approvalRule.email_template_code,
          placeholder,
          requesters
