...
async function getRequesterGroup() {
    try {
        await Promise.all(
            Array.from({ length: 5 }).map(async (_, index) => {
                const res = await client.request.invokeTemplate("getAllRequesterGroup", {
                    context: { page: index + 1 }
                });
...
async function getAgentGroup() {
    try {
        await Promise.all(
            Array.from({ length: 5 }).map(async (_, index) => {
                const res = await client.request.invokeTemplate("getAllAgentGroup", {
                    context: { page: index + 1 }
                });
...
async function getWorkspaces() {
    try {
        const res = await client.request.invokeTemplate("getAllWorkspace", {});
