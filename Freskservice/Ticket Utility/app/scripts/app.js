...
const buttonChangeRequest = document.getElementById("change-request");
...
async function initApp(_client) {
    ...
    requesters = await getRequesters();
    handleEvent();
    receiveData();
...
function handleEvent() {
    ...
    buttonChangeRequest.addEventListener("click", async function () {
        client.interface.trigger("showModal", {
            title: "Change Request",
            template: "modals/change-request.html",
            data: { serviceRequest, requesters }
        });
    });
}
...
function checkType(type) {
    ...
    } else if (type?.includes("PMO - Change Request")) {
        buttonChangeRequest.style.display = "block";
    }
}
